...
class Ctime {
  ...
  //参数定制的时间
  function custom($format = 'Y-m-d H:i:s',$default_time=''){
	  if( $default_time ){
		  return date($format, $default_time + $this->time_difference);
	  }else{
  ...
  //短时间
  function short_time($default_time = ''){
  	if( $default_time ){
	    return date("Y-m-d", $default_time + $this->time_difference);
    }else{
